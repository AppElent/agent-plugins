...
If a skill was reached through `/development:feature apply <feature>`, that run's
...
- **Not about the app's code.** Bugs and ideas about the app you were working in
  are ordinary `/development:project issue` territory, filed against the app's own
...
1. **Check for an existing issue first.**
   `gh issue list --repo AppElent/agent-plugins --state open --search "<terms>"`
...
5. File it with the `issue` procedure in `../../development-project/SKILL.md` —
...
Reflection issues **always** target `AppElent/agent-plugins`, regardless of
...
in `../../development-project/SKILL.md`, which routes `/development:project issue` to
