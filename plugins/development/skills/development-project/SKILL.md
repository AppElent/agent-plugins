...
description: Use when inspecting an app's installed Appelent capabilities and partial adoption, or working on issues in that app's repository.
---

# App capability status

Read the app's instructions, appelent.json, and local guideline index/app.md when present. Use [the feature router](../development-feature/SKILL.md) and its routing catalog to locate FEATURE owners across installed plugins. Missing owners are unavailable evidence, not proof an app lacks a capability.

## list / status [--all]

List recorded IDs, versions, options, and partial step coverage. For status, compare each available FEATURE version and inspect source evidence. Baseline's numbered steps are defined by web-baseline; preserve old selectors. Legacy auth/i18n records do not establish native coverage. Report recorded-without-evidence and implementation-without-record separately.

Compare package declarations with the app's resolved installed versions and known requested release, not an unpublished source checkout version. Package semver differs from FEATURE versions. Run CLI doctor and guidelines check when available; these checks do not prove feature or UX conformance.

Status is read-only. For --all, use projects.json from an explicitly resolved agent-plugins checkout and report missing paths. Do not migrate, stamp, or register apps during status.

For apply/show/steps use /development:feature. For missing owners, identify the needed plugin. For issue/issues/fix use the procedure below.
...
- Via **`/development:project`** (or app-side natural language) → **this app's
...
- Via **`/development:feature`** (or feature/catalog-shaped natural language) →
  `AppElent/agent-plugins` (the catalog repo), fixed. See
  `../development-feature/SKILL.md` for that entry point.
...
procedure — `/development:feature issue` is an alternate entry point to the
...
issues". This is the canonical procedure — `/development:feature issues` is an
...
issue #3", "let's knock out a couple of these". `/development:feature fix` is
...
2. **Small analysis per issue**, done by you, not delegated to a skill:
   ...
   `../development-feature/SKILL.md`), a specific feature's `FEATURE.md`/
...
5. **`just go`** — implement the sketched solution directly in the
   ...
   in the same commit — see the repository ownership guidance in `../development-feature/SKILL.md`. No brainstorming/writing-plans
