...
- **Install (once per machine):** `claude plugin marketplace add AppElent/agent-plugins`, then `claude plugin install appelent@appelent`.
...
- **Web/cloud sessions (any vendor):** the app's own `appelent.json` plus its AGENTS.md/CLAUDE.md managed block point to `github.com/AppElent/agent-plugins`; agents read `FEATURE.md`/`SKILL.md` from GitHub. No committed mirrors.
...
> This is an Appelent-managed app. Opted-in features and their options are recorded in `appelent.json`. Feature definitions live in the `appelent` plugin (locally installed) or `https://github.com/AppElent/agent-plugins` (`skills/<feature>/FEATURE.md`). Before adding functionality that could apply to multiple apps, check the feature catalog first; to add or update a feature, use `/appelent`.
