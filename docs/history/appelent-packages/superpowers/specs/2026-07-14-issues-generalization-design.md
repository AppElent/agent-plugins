...
`AppElent/agent-plugins`, and every issue always carried the same
...
`/web:project` now files against the current app's own GitHub repo;
`/web:feature` keeps filing against the catalog repo; and the label
...
- Via `/web:project` (or app-side natural language) → the current
...
- Via `/web:feature` (or feature/catalog-shaped natural language) →
  `AppElent/agent-plugins`, fixed, same as before.
...
1. From an app checkout, `/web:project issue "settings button is
   misaligned on mobile"` → a `bug`-labeled issue filed in the app's own
   repo, reported as `#N (bug) → <repo>`.
2. `/web:feature issue "baseline apply should also do Y"` → an
   `enhancement`-labeled issue in `AppElent/agent-plugins`.
3. `/web:project issue "README setup steps are out of date"` → a
...
5. `/web:project issues` lists open issues in the app repo; picking
   one seeds `brainstorming` from its title/body. `/web:feature fix
   <n>` runs the triage loop against a catalog issue.
6. `/web:project issue "x"` in a dir with no GitHub `origin` remote →
