...
1. Take the user's one-line idea as the issue title. If the same message
   ...
   empty.
2. Ensure the `catalog-suggestion` label exists on `AppElent/agent-plugins`
   (`gh label list --repo AppElent/agent-plugins` first; create it with
...
4. Always targets `AppElent/agent-plugins` regardless of the current
...
5. After that work concludes (design/plan/implementation as normal),
   ...
   AppElent/agent-plugins`). Never close it automatically — same
