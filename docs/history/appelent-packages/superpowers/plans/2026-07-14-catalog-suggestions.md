...
1. Use the user's one-line idea as the issue title. If their message
   ...
   body empty.
2. Ensure the label exists: `gh label list --repo AppElent/agent-plugins
   --search catalog-suggestion`. If it's not there, create it:
   `gh label create catalog-suggestion --repo AppElent/agent-plugins
   --description "Braindumped improvement idea for the appelent-packages
   catalog mechanism" --color ededed`.
3. File the issue: `gh issue create --repo AppElent/agent-plugins
   --title "<idea>" --body "<context or empty string>" --label
   catalog-suggestion`.
4. Always targets `AppElent/agent-plugins` via `--repo`, regardless of
...
1. List open candidates: `gh issue list --repo AppElent/agent-plugins
...
5. Once that work concludes, offer to close the issue: `gh issue close <n>
   --repo AppElent/agent-plugins`. Never close it without asking —
...
- The exact `gh issue create` command that would run, with `--repo
  AppElent/agent-plugins`, the title, an empty `--body`, and
...
- The `gh issue view 42 --repo AppElent/agent-plugins` command.
...
- Confirm the close offer (`gh issue close 42 --repo
  AppElent/agent-plugins`) only happens after that brainstorming work
...
- **Type consistency:** N/A — no code types; this plan only edits Markdown
  instruction text. The `--repo AppElent/agent-plugins` and
